...
type CommandProps<DmPermission extends boolean> =
  ...
  | {
      ...
      run(
        interaction: C<DmPermission, MessageContextMenuCommandInteraction>
      ): any;
    };

type CommandData<DmPermission extends boolean> = CommandProps<DmPermission> &
  ApplicationCommandData & {
    dmPermission: DmPermission;
  };

export class Command<DmPermission extends boolean = boolean> {
  public static all: Collection<string, CommandData<boolean>> =
    new Collection();
  constructor(public data: CommandData<DmPermission>) {
    log.successComamnd(
      ck.green(
        `${ck.cyan.underline(data.name)} has been successfuly registered!`
      )
    );
