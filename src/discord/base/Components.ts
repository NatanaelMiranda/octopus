...
type ComponentData<Cached extends CacheType = CacheType> =
  ComponentProps<Cached> & {
    cache?: Cached;
    customId: string;
  };
...
export class Component<Cached extends CacheType = CacheType> {
  ...
  public static find<T extends ComponentData["type"]>(
    customId: string,
    type: T
  ) {
    const c = Component.all.find(
      (c) => c.customId === customId && c.type == type
    );
    return c as (ComponentData & { type: T }) | undefined;
  }
  constructor(data: ComponentData<Cached>) {
    log.successComponent(
      ck.green(
        `${ck.cyan.underline(data.customId)} has been successfully saved!`
      )
    );
