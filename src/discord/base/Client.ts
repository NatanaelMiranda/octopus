...
export function createClient(options?: ClientOptions): Client {
    ...
    const onCommand = (commandInteraction: CommandInteraction) => {
      ...
      switch (command?.type) {
        ...
        case ApplicationCommandType.Message: {
          const interaction =
            commandInteraction as MessageContextMenuCommandInteraction;
          command.run(interaction);
          return;
        }
        case ApplicationCommandType.User: {
          const interaction =
            commandInteraction as UserContextMenuCommandInteraction;
          command.run(interaction);
          return;
        }
      }
  ...
  client.once("ready", async (client) => {
    console.log();
    log.success(
      `${ck.green("Bot online")} ${ck.blue.underline(
        "discord.js"
      )} 📦 ${ck.yellow(version)}`
    );
    log.info(
      `${ck.greenBright(
        `➝ Connected with ${ck.underline(client.user.username)}`
      )}`
    );
    console.log();

    await client.application.commands
      .set(Array.from(Command.all.values()))
